...
def std_fig(ax_form=axes_form, rasterize=False):
    fig = plt.figure()
...
def my_histogram(ax, df, var, color='black',label=r'new', density=True, ls='-', var_range=(0,1), units = 1, hatch=''):
    ...
    out = ax.hist(df[var, '']*units, 
              ...
              ls=ls,
               ...
               histtype='step',
                density=density,
                 lw=1.5, zorder=0, rasterized=True)
...
def rescale_muboone_to_SBN(x, channel='nu_e_e', flavor_struct=[0,1,0], dipoles=[0,0,0], GX=0, detector=muboone_numi_absorber):
    ...
    for mN in x:    
        ...
        br_new = get_brs((mN,usqr), channel=channel, flavor_struct=flavor_struct, dipoles=dipoles, GX=GX)
        br_old = get_brs((mN,usqr), channel='nu_e_e', flavor_struct=flavor_struct, dipoles=[0,0,0], GX=0.0)
...
def rescale_muboone_to_SBN_for_dip_vs_usqr(x, mN=0.250, channel='nu_e_e', flavor_struct=[0,1,0], dipoles=[0,0,0], GX=0, detector=muboone_numi_absorber):
    ...
    y=[]
    for dmu in x:    
        ctau0_new=c_LIGHT*get_lifetime((mN,usqr), flavor_struct=flavor_struct, dipoles=[0,dmu,0], GX=GX)
        
        br_new = get_brs((mN,usqr), channel=channel, flavor_struct=flavor_struct, dipoles=[0,dmu,0], GX=GX)
        prob_new = prob_decay_in_interval(L, d, ctau0_new, gamma)*br_new
