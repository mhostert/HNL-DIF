...
# From Ballett et al 1905.00284 appendix A2
def I_lep_minus(x):
	y=0
	return 2*(-(-1 + x)**3*(1 + 3*x) + 8*(-1 + x)**3*y - 6*y**2*(3 - 4*x + x**2 + 2*np.log(x)))


def Rapp(ralpha,rN, parent=lp.K_plus, daughter=lp.mu_plus):
	# three-body muon decay
	if parent == lp.mu_plus or parent == lp.mu_minus:
		# From Ballett et al 1905.00284 eq. 4.4
		R = ma.masked_array(data=I_lep_minus(rN),
							mask = ~(np.sqrt(rN) < 1.0),
							fill_value=0.0)		
	# two-body pi/k decay
	else:	
...
def get_Rapp(mN, parent=lp.K_plus, daughter=lp.mu_plus):
	...
	return Rapp(ralpha,rN,parent=parent)
...
# to be pooled
def get_event_rate(args, flavor_struct=[1.0,1.0,1.0], dipoles={}, dark_coupl={}, 
						detector=exp.nd280, eff=True, modes=['nu_e_e'], 
						parent=lp.K_plus, daughter=lp.mu_plus):
	...
	for this_flavor in detector.prop['flavors']:
		fK = detector.get_flux_func(parent=parent, nuflavor = this_flavor)
		
		if fK:
			fN = lambda pN: dphi_dEN_app(fK, pN, Ualpha4SQR=USQR, mN=M4, parent=parent, daughter=daughter)
