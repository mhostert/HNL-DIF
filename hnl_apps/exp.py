...
class experiment:
    def __init__(self, EXP_FLAG):
        ...
        if self.EXP_FLAG == flag_nd280:
            ...
            nuPOT = 12.34e20
            nubarPOT = 6.29e20
            avg_eff_nu_nubar = (eff[:, 10, :5] * nuPOT + eff[:, 10, 5:] * nubarPOT) / (
                nuPOT + nubarPOT
            )
            summed_avg_eff = np.sum(avg_eff_nu_nubar, axis=1)

            stop_index_fit = 17
            func_extrap = np.polynomial.polynomial.Polynomial.fit(
                masses_t2k[:stop_index_fit], summed_avg_eff[:stop_index_fit], deg=1
            )
            func_interp = interpolate.interp1d(
                masses_t2k, summed_avg_eff, bounds_error=False, fill_value=0
            )
            ...
            self.prop = {
                ...
                "length": 56 * 3,  # cm
                "baseline": 280e2,  # cm
                "eff_nu_e_e": lambda x: np.heaviside(x * 1e3 - 140, 0)
                * func_interp(x * 1e3)
                + np.heaviside(-x * 1e3 + 140, 0) * func_extrap(x * 1e3),
                "emin": 0.05,
                "emax": 10.0,
            ...
            stop_index_fit = 17
            func_extrap = np.polynomial.polynomial.Polynomial.fit(
                masses_t2k[:stop_index_fit], summed_eff[:stop_index_fit], deg=1
            )
            func_interp = interpolate.interp1d(
                masses_t2k, summed_eff, bounds_error=False, fill_value=0
            )
    ...
    def get_flux_func(self, parent=lp.K_plus, nuflavor=lp.nu_mu):
        fluxfile = (
            f"{local_dir}/fluxes/{self.prop['name']}/{nuflavor.name}_{parent.name}.dat"
        )
        try:
            e, f = np.genfromtxt(fluxfile, unpack=True)
        except IOError:
            print(
                f"Fluxfile {fluxfile} not found or not implemented. Skipping this decay."
            )
            return None
        else:
            flux = interpolate.interp1d(
                e, f * self.prop["flux_norm"], fill_value=0.0, bounds_error=False
            )
            return flux
